...
import { CalendarioProfesor, profesorService } from '../../services/profesor.service';

type Estado = 'loading' | 'error' | 'success';

const ProfesorCalendarioView: React.FC = () => {
  const navigate = useNavigate();
  const [estado, setEstado] = useState<Estado>('loading');
  const [data, setData] = useState<CalendarioProfesor | null>(null);
  ...
  const cargar = async () => {
    ...
    setError(null);
    try {
      const resultado = await profesorService.consultarCalendario();
      setData(resultado);
      setEstado('success');
    } catch (err: any) {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: '1200px', background: '#ececec', padding: '20px', border: '1px solid #cfcfcf' }}>
        ...
        {/* SUCCESS sin exámenes asignados */}
          ...
          <div style={{ padding: '10px' }}>
            <div style={{ background: '#fff3cd', border: '2px solid #ffc107', padding: '25px', marginBottom: '28px', textAlign: 'center' }}>
              <div style={{ fontSize: '22px', fontWeight: 'bold', color: '#856404' }}>
                📭 No tiene exámenes asignados
              </div>
              <p style={{ marginTop: '15px', fontSize: '14px', color: '#856404' }}>
                Actualmente no figura ningún examen asignado a su perfil en el calendario.
              </p>
            </div>
            <div style={{ textAlign: 'center' }}>
              <button onClick={cargar} style={{ minWidth: '180px', padding: '12px 20px', borderRadius: '4px', fontSize: '15px', fontFamily: 'inherit', cursor: 'pointer', border: '1px solid #999', fontWeight: 'bold', background: '#2d89ef', color: 'white', marginRight: '15px' }}>
          ...
          <>
            ...
            {data.resumen.conConflicto > 0 && (
              <div style={{ background: '#fff3cd', borderLeft: '4px solid #ffc107', color: '#856404', padding: '12px 15px', marginBottom: '20px', fontSize: '14px' }}>
                ⚠️ <strong>{data.resumen.conConflicto}</strong> de sus exámenes presentan algún conflicto.{' '}
                <button onClick={() => navigate('/profesor/incidencias')} style={{ background: 'none', border: 'none', color: '#2d89ef', cursor: 'pointer', fontFamily: 'inherit', fontSize: '14px', textDecoration: 'underline', padding: 0 }}>
            ...
            <div style={{ border: '2px solid #7ea0c0', background: '#dfe7ef', padding: '15px 20px', marginBottom: '25px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px' }}>
                ...
                {resumenItem('Con conflicto', data.resumen.conConflicto)}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '25px', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => navigate('/profesor/incidencias')} style={{ minWidth: '200px', padding: '12px 20px', borderRadius: '4px', fontSize: '15px', fontFamily: 'inherit', cursor: 'pointer', border: '1px solid #b8860b', fontWeight: 'bold', background: '#e6a017', color: 'white' }}>
...
export default ProfesorCalendarioView;
